    ...
    fn error_response(&self) -> HttpResponse {
        let (status, error_type, message) = match self {
            AppError::InvalidRequest(msg) => {
                (actix_web::http::StatusCode::BAD_REQUEST, "invalid_request_error", msg.clone())
            }
            AppError::UpstreamUnavailable(_) => {
                (actix_web::http::StatusCode::BAD_GATEWAY, "proxy_error", self.to_string())
            }
            AppError::UpstreamError { status, body } => {
            ...
            AppError::ConfigError(_) => {
                (actix_web::http::StatusCode::INTERNAL_SERVER_ERROR, "config_error", self.to_string())
            }
            AppError::JsonError(msg) => {
                (actix_web::http::StatusCode::BAD_REQUEST, "invalid_request_error", msg.clone())
            }
            AppError::Internal(_) => {
                (actix_web::http::StatusCode::INTERNAL_SERVER_ERROR, "internal_error", self.to_string())
            }
        };
